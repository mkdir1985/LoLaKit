#ifndef LOLAKIT_CORE_SPSC_RING_BUFFER_HPP_
#define LOLAKIT_CORE_SPSC_RING_BUFFER_HPP_
...
class SpscRingBuffer : private NonCopyableNonMovable {
 public:
...
template <typename T, std::size_t Capacity>
class TrivialSpscRingBuffer : private NonCopyableNonMovable {
 public:
  static_assert(Capacity >= 2U, "Capacity must be at least 2");
  static_assert((Capacity & (Capacity - 1U)) == 0U,
                "Capacity must be a power of two");
  static_assert(std::is_trivially_copyable<T>::value,
                "TrivialSpscRingBuffer requires trivially copyable T");
  static_assert(std::is_trivially_destructible<T>::value,
                "TrivialSpscRingBuffer requires trivially destructible T");
  static_assert(std::is_trivially_default_constructible<T>::value,
                "TrivialSpscRingBuffer requires trivially default constructible T");

  TrivialSpscRingBuffer() = default;
  ~TrivialSpscRingBuffer() = default;
...
#endif  // LOLAKIT_CORE_SPSC_RING_BUFFER_HPP_
